...
def define_mapi_params():
    return {"mu_N":Parameter(units="[cm^2 / V s]", is_edge=True, valid_range=(0,np.inf)), 
...
def define_rubrene_params():
    return {"mu_P_up":Parameter(units="[cm^2 / V s]", is_edge=True, valid_range=(0,np.inf)),
...
def define_mapi_simulation_outputs():
    return {"N":Output("N", units="[carr / cm^3]", integrated_units="[carr / cm^2]", xlabel="nm", xvar="position", is_edge=False, layer="MAPI", yscale='symlog', yfactors=(1e-4,1e1)), 
...
def define_rubrene_simulation_outputs():
    return {"P_up":Output("P_up", units="[carr / cm^3]", integrated_units="[carr / cm^2]", xlabel="nm", xvar="position",is_edge=False, layer="Rubrene", yscale='symlog', yfactors=(1e-4,1e1)),
...
def define_mapi_calculated_outputs():
    return {"E_field":Output("Electric Field", units="[V/nm]", integrated_units="[V]", xlabel="nm", xvar="position",is_edge=True, layer="MAPI"),
...
def define_rubrene_calculated_outputs():
    return {"E_upc":Output("E (Upc)", units="[V/nm]", integrated_units="[V]", xlabel="nm", xvar="position",is_edge=True, layer="Rubrene"),
...
def define_mapi_convert_in():
    mapi_convert_in = {"mu_N": ((1e7) ** 2) / (1e9),
