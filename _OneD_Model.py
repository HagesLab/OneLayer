...
class OneD_Model:
    ...
    def __init__(self):
        ...
        params = {"Example Parameter": Parameter(
            units="[cm / s]", is_edge=False, valid_range=(0, np.inf))}

        # dict {"Flag's internal name":
        #       ("Flag's display name",
        #        whether Flag1 is toggleable,
        #        Flag1's default value)
        #      }
        self.flags_dict = {"Flag1": ("Flag1's name", 1, 0)}

        simulation_outputs = {"Example Output": Output("Output name",
                                                       units="[cm^-3]",
                                                       integrated_units="[cm^-2]",
                                                       xlabel="cm",
                                                       xvar="position",
                                                       is_edge=False,
                                                       layer="Example Layer",
        ...
        self.layers["Example Layer"] = Layer(params, simulation_outputs,
                                             calculated_outputs, "[nm]",
                                             convert_in, iconvert_in)

        # Modules may additionally define a dummy "__SHARED__" layer for systems
        # in which multiple layers have identical lists of params and outputs
    ...
    def swap_param_rules(self, layer, param_name, i):
        ...
        to_swap = (param.param_rules[i-1], param.param_rules[i])
        param.param_rules[i], param.param_rules[i-1] = to_swap
        self.update_param_toarray(layer, param_name)
        return
    ...
    def removeall_param_rules(self, layer, param_name):
        """
        Deletes all parameter rules and resets stored distribution
    ...
    def update_param_toarray(self, layer, param_name):
        ...
        for condition in param.param_rules:
            ...
            # If the left bound coordinate exceeds the width of
            # the node toIndex() (which always rounds down)
            # assigned, it should actually be mapped to the next node
            distance_to_leftmost_node = (condition.l_bound -
                                         to_pos(i, self.layers[layer].dx,
                                                param.is_edge)
                                         )

            if (distance_to_leftmost_node > self.layers[layer].dx / 2):
                i += 1
            ...
            if (condition.type == "POINT"):
                ...
                j = to_index(condition.r_bound, self.layers[layer].dx,
                             self.layers[layer].total_length, param.is_edge)
                new_param_value[i:j+1] = condition.l_boundval

            elif (condition.type == "LINE"):
                slope = (condition.r_boundval - condition.l_boundval) / \
                    (condition.r_bound - condition.l_bound)
                j = to_index(condition.r_bound, self.layers[layer].dx,
                             self.layers[layer].total_length, param.is_edge)
                ...
                try:
                    new_param_value[i:j+1] = (condition.l_boundval * np.power(
                        condition.r_boundval / condition.l_boundval, ndx / (j - i)))
                except FloatingPointError:
                    logger.info(
                        "Warning: Step size too large to resolve "
        ...
        for layer in self.layers:
            ...
            if self.layers[layer].spacegrid_is_set:
                ...
                if len(self.layers[layer].grid_x_nodes) > 20:
                    ...
                    mssg.append("Nodes: {}".format(
                        self.layers[layer].grid_x_nodes))
                    mssg.append("Edges: {}".format(
                        self.layers[layer].grid_x_edges))
            else:
                mssg.append("Grid is not set")

            for param_name, param in self.layers[layer].params.items():
                mssg.append("{} {}: {}".format(param_name, param.units,
                                               param.value))
            mssg.append("#########")
        return "\n".join(mssg)
    ...
    def report_shared_params(self):
        """
        A shared parameter is one that is tracked
        in every layer defined by a module.
        An example would be mu_N in the PN-junction,
        in which the n-type, buffer, and p-type
    ...
    def report_shared_outputs(self):
        return set.union(*[self.report_shared_s_outputs(),
                           self.report_shared_c_outputs()])

    def calc_inits(self):
        """
        Uses the self.param_dict to calculate initial conditions for ODEINT.

        In many cases this is just returning the appropriate parameter
        from layer's param dict.
    ...
    def simulate(self, data_path, m, n, dt, flags, hmax_,
                 rtol, atol, init_conditions):
        """
        Uses the provided args to call a numerical solver and
        write results to .h5 files.

        No strict rules on how simulate() needs to look or if
        all parameters must be utilised -
        ...
        rtol : float
            Relative tolerance for ODEINT
        atol : float
            Absolute tolerance for ODEINT
    ...
    def get_overview_analysis(self, params, flags, total_time, dt, tsteps,
        ...
        """
        Perform and package all calculations to be displayed on
        TEDs Overview Analysis tab.
        ...
        data_dict : dict {"layer_name" :{"output name": 1D or 2D numpy array}}
            Collection of output data. 1D array if calculated once
            (e.g. an integral over space) and 2D if calculated at multiple times
            ...
            Shared outputs (see report_shared_params()) should go
            into an additional dict with "layer_name" = "__SHARED__".
    ...
    def prep_dataset(self, datatype, target_layer, sim_data, params, flags,
        ...
        """
        Use the raw data in sim_data to calculate
        quantities in self.outputs_dict.
        If datatype is in self.simulated_outputs dict this just needs to
        return the correct item from sim_data
        ...
        sim_data : dict {"datatype": 1D or 2D numpy array}
            Collection of raw data read from .h5 files. 1D if single time step
            or 2D if time and space range.
        ...
        for_integrate : bool, optional
            Whether this function is being called by do_Integrate.
            Used for integration-specific procedures.
            All other optional arguments are needed only
            if for_integrate is True.
        ...
        nen : bool, optional
            Short for "need extra node".
            Whether an extra (the 'j+1'th) node should be counted.
            This is determined by a correction between the node values and
            the actual bounds of the integration.
            See Nanowire.prep_PL and GUI's do_integrate for an example of this.
            The default is False.
        extra_data : dict {"datatype": 2D numpy array}, optional
            A copy of the full set of raw data over all time and space.
            Some integrals like the weighted PL need this.
            The default is None.
    ...
    def get_timeseries(self, pathname, datatype, parent_data, total_time, dt):
        ...
        pathname : str
            ...
            Use a string such as pathname + "-N.h5" to
            access the simulated data.
        datatype : str
            Data type that was just integrated. Use this value to
            associate different time series with related data types.
        parent_data : np.ndarray
            Values that were just integrated. Often the time series can be
            calculated directly from this if a good choice of
            datatype-time series association is made.
        total_time : float
    ...
    def get_IC_regen(self, sim_data, param_dict, include_flags, grid_x):
        """
        Overwrites param_dict with values from the current data analysis
        in preparation for generating a new initial state file.

        Parameters
        ----------
        sim_data : dict {"data type": 1D numpy array}
            Collection of raw data from the currently loaded timestep of the
        ...
        include_flags : dict {"data type": bool}
            Whether to include a data type from self.simulated_outputs_dict in
            the new initial file.
    ...
    def verify(self):
        ...
        if "symmetric_system" not in self.flags_dict:
