...
class ICRegenPopup(Popup):
    ...
    def __init__(self, plot_ID, nb, logger):
        ...
        for var in shared_outputs:
            ...
            self.nb.ICregen_include_flags["__SHARED__"][var].set(0)
            self.regen_checkbuttons["__SHARED__"][var] = \
                tk.Checkbutton(
                    self.toplevel,
                    text=var,
                    variable=self.nb.ICregen_include_flags["__SHARED__"][var])
            self.regen_checkbuttons["__SHARED__"][var].grid(
                row=rcount, column=0)
            rcount += 1
        ...
        for layer_name, layer in self.nb.module.layers.items():
            ...
            for var in layer.s_outputs:
                ...
                self.regen_checkbuttons[layer_name][var].grid(
                    row=rcount, column=0)
                rcount += 1
        ...
        for key, dataset in self.nb.analysis_plots[plot_ID].datagroup.datasets.items():
            over_time = (
                self.nb.analysis_plots[plot_ID].time > dataset.total_time)
            if over_time:
                continue
    ...
    def close(self, logger=None, continue_=False):
        ...
        try:
            if continue_:
                ...
                datasets = [self.regen_IC_listbox.get(
                    i) for i in self.regen_IC_listbox.curselection()]
                if not datasets:
                    return
                ...
                for key in datasets:
                    new_filename = tk.filedialog.asksaveasfilename(initialdir=self.nb.default_dirs["Initial"],
                                                                   title="Save IC text file for {}".format(
                                                                       key),
                    ...
                    pathname = active_sets[key].pathname
                    dirname, header = os.path.split(pathname)
                    ...
                    for layer_name, layer in self.nb.module.layers.items():
                        ...
                        for var in layer.s_outputs:
                            ...
                            if is_shared and shared_done.get(var, False):
                                continue

                            path_name = os.path.join(pathname,
                                                     "{}-{}.h5".format(header, var))
                            floor_tstep = int(
                                active_plot.time / active_sets[key].dt)
                            interpolated_step = u_read(
                                path_name, t0=floor_tstep, t1=floor_tstep+2)

                            if active_plot.time == active_sets[key].total_time:
                                pass
                            else:
                                slope = (
                                    interpolated_step[1] - interpolated_step[0]) / (active_sets[key].dt)
                                interpolated_step = interpolated_step[0] + slope * (
                                    active_plot.time - floor_tstep * active_sets[key].dt)
                    ...
                    for layer_name, layer_params in param_dict_copy.items():
                        for param in layer_params:
                            layer_params[param] *= self.nb.module.layers[layer_name].convert_out.get(
                                param, 1)

                    export_ICfile(new_filename, self.nb, active_sets[key].flags,
                                  param_dict_copy, allow_write_LGC=False)

                    status_msg.append("{}-->{}".format(pathname, new_filename))
            ...
            self.nb.write(self.nb.analysis_status,
                          "Error: failed to regenerate IC file")
