...
class PlotterPopup(Popup):
    ...
    def __init__(self, plot_ID, nb, logger):
        ...
        if len(self.nb.module.layers) > 1:
            shared_outputs = set.union(
                *[self.nb.module.report_shared_s_outputs(), self.nb.module.report_shared_c_outputs()])
        else:
            shared_outputs = {}
        ...
        try:
            ...
            if continue_:
                ...
                for next_dir in self.data_list:
                    self.nb.analysis_plots[plot_ID].data_pathnames.append(
                        next_dir)
